$(document).ready(function() {
    ...
    var coleccionProductos = db.ref().child("Recepcion_Fruta");
    ...
    coleccionProductos.on("child_added", datos => {        
        dataSet = [datos.key, datos.child("id").val(), datos.child("fecha_recepcion").val(), datos.child("productor").val(), datos.child("placas").val(), datos.child("localidad").val(), datos.child("kg_recibidos").val(), datos.child("no_rejas").val()];
    ...
    $("#tablaProductos").on("click", ".btnBorrar", function() {   
        ...
        if (result.value) {
            ...
            let id = fila[0];  
            console.log(id);          
            db.ref(`Recepcion_Fruta/${id}`).remove()
